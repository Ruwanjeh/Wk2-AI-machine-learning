...
page = st.sidebar.radio(
    "Go to", ["Overview", "Regional Analysis", "Predictions"])
...
if page == "Overview":
    ...
    with col3:
        st.metric("Latest Year Cases",
                  f"{yearly_data['No. of cases'].iloc[-1]:,.0f}")
    ...
    st.write(
        f"Total deaths in most recent year: {yearly_data['No. of deaths'].iloc[-1]:,.0f}")
    st.write(
        f"Average case fatality rate: {yearly_data['case_fatality_rate'].mean():.2f}%")
    ...
    try:
        ...
        with col1:
            year = st.number_input(
                "Year", min_value=2010, max_value=2030, value=2023)
        with col2:
            region = st.selectbox("WHO Region", df['WHO Region'].unique())

        if st.button("Predict"):
            input_data = pd.DataFrame({'Year': [year], 'WHO Region': [region]})
            input_data = pd.get_dummies(input_data)
            input_data = input_data.reindex(
                columns=model_columns, fill_value=0)
            st.write("DEBUG: Input to model", input_data)
